...
class BatchPipelineRunner:
    """
    Batch runner to execute MD pipeline across multiple protein-ligand pairs.
    Supports CSV/JSON manifest files and provides comprehensive summary reports.
    """
    ...
    def run_batch(
        ...
        for idx, comp in enumerate(self.complexes, start=1):
            comp_name = comp.get("ligand_name") or f"complex_{idx}"
            work_dir = comp.get("work_dir") or f"simulations/{comp_name}"
            ...
            summary_item = {
                ...
                "residue_name": comp.get("residue_name", "UNL"),
                "work_dir": work_dir,
                "status": "PENDING",
                "step_reached": 0,
                "elapsed_sec": 0.0,
            ...
            try:
                pipeline = ProteinLigandPipeline(
                    ...
                    residue_name=comp.get("residue_name", "UNL"),
                    work_dir=work_dir,
                    ph=_parse_float(comp.get("ph")),
                    full_protonate=_parse_bool(comp.get("full_protonate")),
                    prepare_top=_parse_bool(comp.get("prepare_top")),
                    ff=comp.get("ff", "amber99sb-ildn"),
                    ion_conc=_parse_float(comp.get("ion_conc"), 0.15),
                    box_dist=_parse_float(comp.get("box_dist"), 1.5),
                    box_type=comp.get("box_type", "dodecahedron"),
                    prod_time_ns=_parse_float(comp.get("prod_time_ns"), 100.0),
                )
                ...
                pipeline.run_step2(
                    net_charge=_parse_int(comp.get("net_charge"), 0),
                    charge_method=comp.get("charge_method", "bcc"),
                )
                summary_item["step_reached"] = 2

                if _parse_bool(comp.get("run_invacuo")):
                    pipeline.run_step3()
                ...
                if up_to_step >= 7:
                    pipeline.run_step7(
                        nt=_parse_int(comp.get("nt_em"), 1),
                        gpu=_parse_bool(comp.get("gpu_em")),
                    )
                    summary_item["step_reached"] = 7

                if up_to_step >= 8:
                    pipeline.run_step8(
                        n_cores=_parse_int(comp.get("n_cores_md")),
                        gpu_id=str(comp.get("gpu_id_md", "0")),
                    )
...
def batch_cli():
    ...
    parser = argparse.ArgumentParser(description="Run GROMACS protein-ligand pipeline in batch mode from manifest.")
    parser.add_argument("-m", "--manifest", type=str, required=True, help="Path to CSV or JSON manifest")
    parser.add_argument("--up-to-step", "--up_to_step", type=int, default=7, help="Step to run up to (default: 7)")
    parser.add_argument("--stop-on-error", action="store_true", help="Stop batch on first error")
    parser.add_argument("--out-dir", type=str, default=".", help="Directory to save batch summary files")
    ...
    runner = BatchPipelineRunner(manifest=args.manifest)
    runner.run_batch(
        up_to_step=args.up_to_step,
        continue_on_error=not args.stop_on_error,
        output_summary_dir=args.out_dir,
    )
