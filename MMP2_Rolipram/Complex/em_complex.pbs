...
export MDNAME=em
export GROFILE=solv_ions.gro
export MDPFILE=em.mdp
export TOPOL_FILE=topol.top

SSHBIN=/usr/bin/ssh

#Preprocessing: Need to mount the Rolipram dir for the ligand topology data
$SSHBIN $GPUHOST LD_LIBRARY_PATH="" singularity run --nv -B ${PWD}/../Rolipram:/Rolipram -B ${PWD}:/host_pwd --pwd /host_pwd $SIFPATH/$SIFIMG gmx grompp -f $MDPFILE -c $GROFILE -p $TOPOL_FILE -o ${MDNAME}.tpr -maxwarn 2
